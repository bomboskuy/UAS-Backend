...
func (r *achievementReferenceRepositoryPg) UpdateStatus(id string, status string, verifierID *string, note *string) error {
	now := time.Now()

	if status == "submitted" {
		query := `UPDATE achievement_references SET status=$1, submitted_at=$2, updated_at=$3 WHERE id=$4`
		_, err := r.db.Exec(query, status, now, now, id)
		return err
	}

	if status == "verified" {
		query := `UPDATE achievement_references SET status=$1, verified_at=$2, verified_by=$3, updated_at=$4 WHERE id=$5`
		_, err := r.db.Exec(query, status, now, verifierID, now, id)
		return err
	}

	if status == "rejected" {
		query := `UPDATE achievement_references SET status=$1, rejection_note=$2, updated_at=$3 WHERE id=$4`
		_, err := r.db.Exec(query, status, note, now, id)
		return err
	}
...
func (r *achievementReferenceRepositoryPg) Delete(id string) error {
